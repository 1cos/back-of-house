...
function renderSuggBlock(sugg, i) {
  ...
  // ── EXECUTIVE SUMMARY (boh-v711) ─────────────────────────────────────────
  // Deterministic operational briefing — generated from existing fields only.
  // No AI, no new queries, no DB changes. Shown at the very top of every card.
  // Written like a sous chef addressing the Executive Chef.
  function _buildExecSummary() {
    let headline = '';
    let reasons  = [];
    let bgColor  = 'rgba(248,250,252,0.9)';
    let border   = '#e2e8f0';
    let textColor= '#334155';

    // ── STOCK VERIFIED (overrides all other states) ────────────────────────
    if (_stockVerifiedOk) {
      headline = 'No production needed.';
      const _svCnt = (window._recentCounts || {})[i.id];
      if (_svCnt && _svCnt.counted_by) {
        const _svT = _svCnt.counted_at
          ? new Date(_svCnt.counted_at).toLocaleTimeString('en-US',
              {hour:'2-digit', minute:'2-digit', hour12:true, timeZone:'America/Chicago'})
          : '';
        reasons.push('Stock was physically verified by ' + _svCnt.counted_by + (_svT ? ' at ' + _svT : '') + '.');
      } else {
        reasons.push('Physical stock count confirms levels are sufficient.');
      }
      reasons.push('Current stock covers expected demand for the coverage window.');
      bgColor = 'rgba(240,253,244,0.95)';
      border  = '#bbf7d0';
      textColor = '#166534';
      return _renderExecBlock(headline, reasons, bgColor, border, textColor);
    }

    // ── OPERATIONAL (Thaw tasks) ───────────────────────────────────────────
    if (isOperational) {
      headline = 'Check the freezer.';
      reasons.push('This item needs to be thawed before service — no batch production required.');
      if (displayLabel) {
        reasons.push('Estimated need: ' + displayLabel + '.');
      } else {
        reasons.push('Check current levels and pull what you need for tonight.');
      }
    ...
    // ── DO FIRST (stock zero or critically low) ────────────────────────────
    if (status === 'do_first') {
      if (displayLabel) {
        headline = 'Produce ' + displayLabel + ' before anything else.';
      } else if (isBlocked) {
        headline = 'Production needed — check stock and prep as required.';
      } else {
        headline = 'This prep needs to happen before service.';
      }
      const _liveS = _liveStockForRec;
      if (_liveS !== null && !isNaN(_liveS) && _liveS === 0) {
        reasons.push('Current stock is at zero.');
      } else if (_liveS !== null && !isNaN(_liveS) && _liveS > 0) {
        const _su = sugg.stock_source === 'db_snapshot_unverified';
        reasons.push('Recorded stock is low' + (_su ? ' — physical count recommended.' : '.'));
      } else {
        reasons.push('Current stock is insufficient for the coverage window.');
      }
      if (sugg.stock_source === 'db_snapshot_unverified') {
        reasons.push('Stock level has not been physically verified today.');
      }
      const _al = _avgDailyLabel(sugg, outUnit);
      if (_al) {
        reasons.push('Average daily usage is ' + _al + '.');
      }
    ...
    // ── PREP TODAY (stock low but some available) ──────────────────────────
    if (status === 'prep_today') {
      if (displayLabel) {
        headline = 'Plan to produce ' + displayLabel + ' today.';
      } else if (isBlocked) {
        headline = 'Some production needed today — verify stock levels.';
      } else {
        headline = 'Stock is running low. Prepare before service.';
      }
      const _liveS2 = _liveStockForRec;
      if (_liveS2 !== null && !isNaN(_liveS2) && _liveS2 > 0 && stockStr) {
      ...
      if (sugg.stock_source === 'db_snapshot_unverified') {
        reasons.push('Please check the cooler before starting production.');
      }
      const _al2 = _avgDailyLabel(sugg, outUnit);
      if (_al2) {
        reasons.push('Running at approximately ' + _al2 + '.');
      }
    ...
    if (status === 'looks_ok') {
      ...
      const _al3 = _avgDailyLabel(sugg, outUnit);
      if (_al3) {
        reasons.push('Average usage is ' + _al3 + ' — stock is comfortably ahead.');
      } else {
        reasons.push('No additional prep required at this time.');
      }
      bgColor = 'rgba(240,253,244,0.95)';
      border  = '#bbf7d0';
      textColor = '#166534';
    ...
    // ── COUNT FIRST (stock not verified / unknown) ─────────────────────────
    if (status === 'count_first') {
      headline = 'Count the stock before producing.';
      reasons.push('The system cannot confirm the current inventory for this item.');
      if (sugg.current_stock === null || sugg.current_stock === undefined) {
        reasons.push('No stock reading has been recorded for this prep yet.');
      } else {
        reasons.push('The recorded level may be stale — a physical count is needed first.');
      }
      reasons.push('Use Align Stock after counting so the system can give an accurate recommendation.');
    ...
    // ── NO DEMAND PATH / OUT OF SCOPE / UNKNOWN ────────────────────────────
    headline = 'No recommendation available.';
    reasons.push('This prep has no linked sales data or demand path in the system.');
    reasons.push('Use your judgment based on current stock and expected covers.');
  ...
  function _renderExecBlock(headline, reasons, bg, bdr, color) {
    const reasonItems = reasons.slice(0, 3).map(function(r) {
      return '<div style="font-size:12px;color:#475569;margin-top:3px;line-height:1.45;padding-left:10px;border-left:2px solid ' + bdr + ';">' + r + '</div>';
    }).join('');
    return '<div style="margin-bottom:10px;padding:10px 12px;border-radius:12px;background:' + bg + ';border:1px solid ' + bdr + ';">'
      + '<div style="font-size:13px;font-weight:800;color:' + color + ';letter-spacing:-0.01em;line-height:1.3;">' + headline + '</div>'
      + reasonItems
      + '</div>';
  }

  const execSummaryHtml = _buildExecSummary();
  ...
  if (_isPilotStatus) {
    ...
    return `<div style="margin-top:4px;">${execSummaryHtml}${pillHtml}${_onHandHtml}${_todayHtml}${_svInlineHtml}${_detailsHtml}${_alignClosedHtml}${_useStockClosedHtml}${_startClosedHtml}</div>`;
  }
  // ── END CLOSED CARD PILOT ─────────────────────────────────────────────────

  return `<div style="margin-top:4px;">${execSummaryHtml}${pillHtml}${actionQtyHtml}${descHtml}${rcHtml}${stockHtml}${avgHtml}${confHtml}${trustHtml}${alignStockBtnHtml}${useStockBtnHtml}</div>`;
}
