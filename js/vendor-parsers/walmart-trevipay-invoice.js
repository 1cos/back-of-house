...
function parseRowStart(line) {
  ...
  let vendorSku = leadToken;
  let description = descFirst;
  ...
  if (/^shipping$/i.test(leadToken)) {
    ...
    // Shipping, nor the adjustment placeholder) — reject so the caller
    // falls through to continuation handling instead of misfiling
    // unrelated text as a new product row.
    return null;
  }
