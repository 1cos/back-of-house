...
// ══════════════════════════════════════════════════════════════
// SOUS CHEF CHAT — tap breve sul microfono
// Chat privata Max ↔ Sous Chef con accesso completo al DB
// ══════════════════════════════════════════════════════════════

let _scChatHistory = []; // cronologia sessione
    ...
    ">
      ...
      <div style="padding:12px 16px 8px;border-bottom:1px solid #f1f5f9;flex-shrink:0;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;">
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            <button onclick="runSousChefScan()" title="Scan warning"
              style="width:36px;height:36px;border-radius:10px;background:#f8fafc;border:1px solid #e2e8f0;font-size:16px;cursor:pointer;">🔍</button>
      ...
      <div id="_scChatMsgs" style="flex:1;overflow-y:auto;padding:12px 14px;display:flex;flex-direction:column;gap:10px;">
        ${_scChatHistory.length === 0 ? `
          <div style="text-align:center;padding:24px 16px;color:#94a3b8;">
            ...
            <div style="font-size:13px;line-height:1.5;">Chiedimi qualsiasi cosa — prezzi, fornitori, vendite, ricette. Posso anche aggiornare il database.</div>
          </div>
          <div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;padding:0 8px;">
            ${['Quanto costa la burrata?','Stew Meat: 12 lb a cassa, $3.29/lb','Confronta prezzi mozzarella','Quante uova abbiamo ordinato?'].map(s => `
      ...
      <div style="padding:10px 12px 20px;border-top:1px solid #f1f5f9;flex-shrink:0;">
        <div style="display:flex;gap:8px;align-items:flex-end;">
          <textarea id="_scChatInput" placeholder="Chiedi al Sous Chef..."
            ...
            onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();scChatSend();}"
          ></textarea>
          <button onclick="scChatSend()"
            style="width:48px;height:48px;border-radius:14px;background:#1e293b;color:white;border:none;cursor:pointer;font-size:20px;flex-shrink:0;">
            ↑
          </button>
          <button onclick="scChatVoice()"
            style="width:48px;height:48px;border-radius:14px;background:#3b82f6;color:white;border:none;cursor:pointer;font-size:20px;flex-shrink:0;">
            🎙
          </button>
        </div>
      </div>
...
window.scChatVoice = async function() {
  ...
  // Avvia registrazione — quando finisce, manda alla chat invece che a classifyWithGroq
  window._scChatVoiceMode = true;
  await startRecording();
  // Modifica stopRecording per redirectare alla chat
  const origStop = window._scOrigStopRecording || stopRecording;
  window._scOrigStopRecording = origStop;
};

// ── Processa messaggio con AI + DB ───────────────────────────
async function scChatProcess(userText) {
  ...
  try {
    // 1. Raccogli contesto DB completo
    const dbContext = await scChatFetchContext(userText, sb);

    // 2. Chiedi a OpenRouter
    ...
    // 3. Esegui azioni DB se richieste
    let dbActionMsg = null;
    if (data.action && sb) {
      dbActionMsg = await scChatExecuteAction(data.action, sb);
    }

    scChatAddMsg('assistant', data.reply, { dbAction: dbActionMsg });
...
// ── Esegui azione DB ────────────────────────────────────────
async function scChatExecuteAction(action, sb) {
  try {
    if (action.type === 'update_ingredient_vendor') {
      const { ingredient_name, vendor, updates } = action;
      // Trova ingredient_id
      const { data: ingr } = await sb
        .from('ingredients')
        .select('id')
        .ilike('name', `%${ingredient_name}%`)
        .limit(1);
      if (!ingr?.length) return `❌ Ingrediente "${ingredient_name}" non trovato`;

      // Ricalcola price_per_100g se necessario
      if (updates.unit_price || updates.conversion_to_base || updates.price_type) {
        const { data: existing } = await sb
          .from('ingredient_vendors')
          .select('*')
          .eq('ingredient_id', ingr[0].id)
          .ilike('vendor', `%${vendor || ''}%`)
          .limit(1);
        if (existing?.length) {
          ...
          else if (merged.conversion_to_base) p100 = (up / parseFloat(merged.conversion_to_base)) * 100;
          if (p100) updates.price_per_100g = parseFloat(p100.toFixed(4));
        }
      }
      ...
      if (error) return '❌ Errore DB: ' + error.message;
      if (typeof loadWarningsBanner === 'function') loadWarningsBanner();
      return `Aggiornato ${ingredient_name}`;
    }

    if (action.type === 'create_task') {
      const { category, urgency, summary, due_date } = action;
      await sb.from('sous_chef_tasks').insert({
        user_name: window.user?.name || 'Admin',
        type: 'task', category, urgency: urgency || 'media',
        summary, text: summary, done: false,
        due_date: due_date || null,
      });
      return `Task creato: ${summary}`;
    }
