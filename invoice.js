...
// ── STANDARD UNIT WEIGHTS (TASK 0b) ──────────────────────────
// Applied automatically for known count-only items.
// Keys are lowercase keyword patterns to match against item description.
// Value is weight in grams per single unit.
const STANDARD_UNIT_WEIGHTS=[
  {keywords:['egg','eggs'],   gPerUnit:58,  label:'large egg'},
  {keywords:['lime','limes'], gPerUnit:67,  label:'lime'},
  {keywords:['lemon','lemons'],gPerUnit:100, label:'lemon'},
  {keywords:['romaine','romana','rom'],gPerUnit:350,label:'romaine head'},
  {keywords:['flower','flowers','floral','bloom','blooms'],gPerUnit:3,label:'flower'},
  {keywords:['avocado','avocados'],gPerUnit:200,label:'avocado'},
  {keywords:['artichoke','artichokes'],gPerUnit:280,label:'artichoke'},
  {keywords:['onion','onions'],gPerUnit:170,label:'onion'},
  {keywords:['garlic head','garlic bulb'],gPerUnit:50,label:'garlic head'},
  {keywords:['bell pepper','bell peppers'],gPerUnit:160,label:'bell pepper'},
  {keywords:['jalapen','jalapeno'],gPerUnit:45,label:'jalapeño'},
  {keywords:['potato','potatoes'],gPerUnit:170,label:'potato'},
];

/**
 * Try to look up a standard unit weight for a count-only invoice item.
 * Returns grams per single unit, or null if unknown.
 */
function lookupStandardUnitWeight(item){
  const desc=(item.description||'').toLowerCase();
  const pack=(item.pack_size||item.pack_description||'').toLowerCase();
  for(const entry of STANDARD_UNIT_WEIGHTS){
    if(entry.keywords.some(kw=>desc.includes(kw)||pack.includes(kw))){
      return {gPerUnit:entry.gPerUnit,label:entry.label};
    }
  }
  return null;
}
...
function enrichInvoiceItems(data){
  (data.items||[]).forEach(item=>{
    let totalG=calcTotalWeightG(item);

    // ── TASK 0b: apply standard unit weights for known count-only items ──
    if(!totalG&&!item.needs_clarification){
      const pack=item.pack_size||item.pack_description||'';
      const isCountOnly=/\b(ct|ea|each|pc|pcs|count|dz|dozen)\b/i.test(pack)&&!/\b(lb|oz|g|kg|gal|qt|pt|ml|l)\b/i.test(pack);
      const pu=(item.purchase_unit||item.unit||'').toLowerCase();
      const isEachUnit=(pu==='each'||pu==='cs'||pu==='case'||!pu);

      if(isCountOnly||isEachUnit){
        const match=lookupStandardUnitWeight(item);
        if(match){
          // Determine total unit count from pack description
          const parsedPack=parsePackFormat(pack);
          const unitCount=parsedPack&&parsedPack.unit==='each'
            ? parsedPack.count*(parsedPack.sizeEach||1)
            : (parsedPack?.count||1);
          const qty=parseFloat(item.quantity||item.qty)||1;
          totalG=qty*unitCount*match.gPerUnit;
          item.avg_unit_weight_g=match.gPerUnit;
          item._standard_weight_label=match.label;
          console.log(`[TASK-0b] Applied standard weight for "${item.description}": ${match.gPerUnit}g/${match.label} → totalG=${totalG}`);
        }
      }
    }
...
function showInvoicePreview(data){
  ...
  const itemsHtml=(data.items||[]).map(item=>{
    const weightLine=item._total_weight_g
      ?`<span style="color:#10b981;">${item._total_weight_g>=1000?(item._total_weight_g/1000).toFixed(2)+'kg':Math.round(item._total_weight_g)+'g'}${item._standard_weight_label?' ('+item._standard_weight_label+')':''}</span>`
...
// ── YES CHEF MODAL (TASK 0a) ──────────────────────────────────
// Replaces the "Invoice saved" header with a celebratory moment.
async function showSaveSuccessModal({data,linesCreated,linesError,ingr=[]}){
  ...
  // Build Yes Chef summary counts
  const allItems=data.items||[];
  const knownCount=allItems.filter(i=>i._match_status==='ok'||i._match_status==='suggest'||i._match_status==='price_up'||i._match_status==='price_down').length;
  const brandNewItems=allItems.filter(i=>i._match_status==='new');
  const brandNewCount=brandNewItems.length;
  const totalItems=allItems.length;
  const invoiceTotal=data.total!=null?'$'+(data.total||0).toFixed(2):null;
  const invoiceDate=data.invoice_date||null;
  const vendor=data.vendor||'Unknown Vendor';
  const invoiceNum=data.invoice_number?'#'+data.invoice_number:null;

  // New item name list (up to 5, then "and N more")
  const MAX_NEW_NAMES=5;
  let newNamesHtml='';
  if(brandNewCount>0){
    const shown=brandNewItems.slice(0,MAX_NEW_NAMES).map(i=>`
      <div style="display:flex;align-items:center;gap:6px;padding:5px 0;border-bottom:0.5px solid rgba(245,158,11,0.12);">
        <span style="font-size:14px;">🆕</span>
        <span style="font-size:12px;color:#92400e;font-weight:500;">${i.description||''}</span>
      </div>`).join('');
    const extra=brandNewCount>MAX_NEW_NAMES
      ?`<div style="font-size:11px;color:#b45309;padding:4px 0;">…and ${brandNewCount-MAX_NEW_NAMES} more</div>`:'';
    newNamesHtml=`
      <div style="background:rgba(245,158,11,0.06);border:0.5px solid rgba(245,158,11,0.25);border-radius:12px;padding:10px 12px;margin-bottom:12px;">
        <div style="font-size:10px;font-weight:700;color:#b45309;letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px;">New items — needs matching</div>
        ${shown}${extra}
      </div>`;
  }

  const allOk=allItems.every(i=>i._match_status==='ok');

  modal.innerHTML=`
    <div style="background:rgba(255,255,255,0.98);border-radius:24px 24px 0 0;padding:20px;width:100%;max-width:480px;margin:0 auto;max-height:88vh;overflow-y:auto;">

      <!-- YES CHEF HEADER -->
      <div style="text-align:center;padding:8px 0 18px;">
        <div style="font-size:44px;margin-bottom:6px;">✅</div>
        <div style="font-size:26px;font-weight:700;color:#065f46;letter-spacing:-.5px;">Yes Chef!</div>
        <div style="font-size:13px;color:#10b981;margin-top:4px;">Invoice saved and ready.</div>
      </div>

      <!-- INVOICE SUMMARY CARD -->
      <div style="background:rgba(16,185,129,0.07);border:0.5px solid rgba(16,185,129,0.22);border-radius:16px;padding:14px;margin-bottom:14px;">
        <div style="font-size:15px;font-weight:600;color:#1e3a5f;margin-bottom:8px;">${vendor}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
          <div style="text-align:center;">
            <div style="font-size:20px;font-weight:700;color:#065f46;">${totalItems}</div>
            <div style="font-size:10px;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;">Items</div>
          </div>
          <div style="text-align:center;">
            <div style="font-size:20px;font-weight:700;color:#065f46;">${knownCount}</div>
            <div style="font-size:10px;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;">Known</div>
          </div>
          <div style="text-align:center;">
            <div style="font-size:20px;font-weight:700;color:${brandNewCount>0?'#d97706':'#065f46'};">${brandNewCount}</div>
            <div style="font-size:10px;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;">New</div>
          </div>
        </div>
        ${invoiceDate||invoiceNum||invoiceTotal?`
        <div style="margin-top:10px;padding-top:10px;border-top:0.5px solid rgba(16,185,129,0.18);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px;">
          <div style="font-size:12px;color:#475569;">${[invoiceNum,invoiceDate].filter(Boolean).join(' · ')}</div>
          ${invoiceTotal?`<div style="font-size:16px;font-weight:700;color:#1e3a5f;">${invoiceTotal}</div>`:''}
        </div>`:''}
      </div>

      <!-- NEW ITEMS LIST (if any) -->
      ${newNamesHtml}

      <!-- ITEM CARDS (suggest / price changes / new) -->
      <div id="_ssItems" style="margin-bottom:14px;"></div>

      <!-- GOT IT BUTTON -->
      <button id="_ssDone" style="width:100%;height:52px;border-radius:16px;background:${allOk?'#10b981':'#1e3a5f'};color:white;font-size:16px;font-weight:600;border:none;cursor:pointer;letter-spacing:.01em;">
        ${allOk?'✅ Got it — back to kitchen':'Done — review items above'}
      </button>
    </div>`;
  ...
  function refreshDoneButton(data,modal){
    ...
    if(!pending.length){
      ...
      doneBtn.textContent='✅ Got it — back to kitchen';
    }
  }
  ...
  // Render item cards (only non-ok items need attention)
  const container=document.getElementById('_ssItems');
  (data.items||[]).filter(i=>i._match_status!=='ok').forEach(function(item){
...
async function linkItemToIngredient(item,ingredientName,data,modal,knownId){
  ...
  function refreshDoneButton(data,modal){
    ...
    const doneBtn=document.getElementById('_ssDone');
    if(!doneBtn)return;
    if(!pending.length){doneBtn.style.background='#10b981';doneBtn.textContent='✅ Got it — back to kitchen';}
  }
}
...
async function saveAllMatches(vendor){
  ...
  // ── Summary toast (saveAllMatches path — no Yes Chef modal here) ──
  const parts=[
...
function showWeightQuestionModal(item,invoiceData,onComplete){
  ...
  window.answerWeightQuestion=async(grams,btn)=>{
    ...
    if(grams&&grams>0){
      ...
      // Save answer to clarifications so next time it's automatic (OQR-009 "first time ask, second time learn")
      if(invData.vendor&&itm.description){
        supa.from('invoice_line_clarifications').upsert({
          vendor:invData.vendor,raw_description:itm.description,
          question:'unit_weight_g',answer:String(grams),
          resolution:{avg_unit_weight_g:grams,_weight_answered:true},
          answered_by:user?.name||'Max'
        },{onConflict:'vendor,raw_description'}).then(({error})=>{
          if(error) console.warn('Weight clarification save error:',error);
        });
      }
    } else {
