...
async function writeInvoiceLines(
  ...
  const invoiceLineRows = items
    ...
    // line with quantity 0). Lo impostano il parser BEK e, da INV08B, il
    // parser Hardie's per gli articoli con shipped 0; gli articoli senza
    ...
    .map((item: any) => {
      ...
      // INV08B — RICEVUTO prima di ORDINATO.
      // invoice_lines registra beni effettivamente ricevuti, quindi fra i
      // due fallback vince quello che descrive la consegna. L'ordinato
      // resta ultimo: serve solo quando il ricevuto non e' noto (NULL),
      // come nelle order confirmation, dove l'unico dato esistente e'
      // appunto la quantita' ordinata.
      const qty = item.catchweight === true ? 1 : item.qty != null ? item.qty : item.qty_received != null ? item.qty_received : item.qty_ordered != null ? item.qty_ordered : null;
      ...
      const cwGrams = item.catchweight
        ...
        : null;
      const per100g = item._cost_per_100g ? parseFloat(item._cost_per_100g)
        : item.cost_per_lb ? (item.cost_per_lb / 453.592) * 100
        : item.catchweight
