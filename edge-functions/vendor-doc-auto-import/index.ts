...
async function processOneQueuedDoc(sb: any, doc: any, parsers: any): Promise<{ outcome: string; detail?: string }> {
  ...
  // ══════════════════════════════════════════════════════════════════
  // MICRO-TASK 48 — BEN E. KEITH BUYER GUARD
  //
  // BEK serves two order streams on the SAME Customer# FDF770366: the
  // kitchen brigade's and the front of house's. Measured on 56 real Gmail
  // threads, Customer#, Branch, Customer Name, subject, sender and
  // recipient are identical on every one — only the `Email:` line in the
  // confirmation header differs. The canonical parser extracts it as
  // buyer_email and decides buyer_class; this is where that decision
  // becomes an outcome.
  //
  // Placed BEFORE the revision/dedup rules on purpose: a front-of-house
  // order must not take part in the kitchen's Sales Order reconciliation
  // at all, not even as a superseded sibling.
  //
  // Allow-list, FAIL CLOSED: only the known kitchen buyer proceeds.
  // ══════════════════════════════════════════════════════════════════
  if (parsersApi().isPurchasableDocument(parsed.vendor, parsed.document_type)
      && parsersApi().isBenEKeith(parsed.vendor)) {
    const buyerClass = parsed.buyer_class || parsersApi().BUYER_UNKNOWN;
    const keepJson = { ...parsed, source: doc.parsed_json?.source, storage_path: storagePath };

    if (buyerClass === parsersApi().BUYER_EXCLUDED) {
      // A real order, just not the chef's. Kept for audit under the same
      // 'ignored' status MICRO-TASK 42 already uses for superseded BEK
      // revisions — never a purchase, and never surfaced as an unmatched
      // ingredient for someone to resolve.
      await sb.from('vendor_documents').update({
        status: 'ignored',
        ...
        warnings: [{
          ...
          message: `Order placed by ${parsed.buyer_email} — front of house, not a kitchen purchase`,
    ...
    if (buyerClass !== parsersApi().BUYER_KITCHEN) {
      // Missing or unrecognised buyer. NOT assumed to be either side.
      // Left pending with a blocking warning so it can never auto-import
      ...
      await sb.from('vendor_documents').update({
        ...
        warnings: [{
          ...
          message: parsed.buyer_email
            ? `Unrecognised Ben E. Keith buyer "${parsed.buyer_email}" — refusing to guess whether this is a kitchen purchase`
  ...
  // ══════════════════════════════════════════════════════════════════
  // MICRO-TASK 42, section F — Ben E. Keith: Sales Order is a
  // RECONCILIATION GROUP, not a unique raw message.
  //
  // The generic duplicate check below is first-wins: the second document
  // with the same vendor+number+type is killed as DUPLICATE. For BEK that
  // is exactly backwards. Real production case 0003126637:
  //
  //   04 Sep  acknowledgement, 4x Requested, confirmed 0  → NOT a purchase
  //   06 Sep  operational,     4x Filled,    confirmed 1  → THE purchase
  //
  // First-wins would let the acknowledgement claim the Sales Order and
  // mark the real purchase as a duplicate error. So BEK gets its own rule,
  // placed BEFORE the generic check, and only ever reaches one outcome:
  //
  //   a purchase already imported for this Sales Order
  //     → FAIL CLOSED. Never a second purchase, never a silent rewrite of
  //       an already-accounted one. Held as a blocking exception.
  //   no purchase imported yet
  //     → the newest revision is the operative one; older siblings are
  //       superseded to 'ignored'. Revisions stay identifiable by
  //       created_at and source_email_subject.
  // ══════════════════════════════════════════════════════════════════
  if (docNumber && parsersApi().isBenEKeith(parsed.vendor) && parsed.document_type === 'order_confirmation') {
    ...
    const rows = siblings || [];
    const alreadyImported = rows.find((r: any) => r.status === 'imported');
    if (alreadyImported) {
      await sb.from('vendor_documents').update({
        status: 'pending',
        warnings: [{
