...
function renderRecipes(){
  ...
  document.getElementById('recipeCats').innerHTML = cats.map(c=>{
    const active = recipeCat===c;
    return `<button onclick="recipeCat='${c.replace(/'/g,"\\'")}';renderRecipes()"
      style="height:30px;padding:0 14px;border-radius:15px;white-space:nowrap;font-size:13px;font-weight:${active?'600':'400'};cursor:pointer;-webkit-tap-highlight-color:transparent;flex-shrink:0;background:${active?'#1e3a5f':'rgba(255,255,255,0.6)'};color:${active?'white':'#1e3a5f'};border:0.5px solid ${active?'#1e3a5f':'rgba(59,130,246,0.2)'};">${c}</button>`;
  }).join('');
  ...
  grid.innerHTML = filtered.map((r,idx)=>{
    ...
    const yieldTxt = (r.yield_text||r.yield||'1 serving')
      .replace(/porzioni|portions|porciones/gi,{'it':'porzioni','en':'portions','es':'porciones'}[user?.lang||'en']||'portions')
      .replace(/porzione|serving|porcion/gi,{'it':'porzione','en':'serving','es':'porcion'}[user?.lang||'en']||'serving');
    ...
    return `<div onclick="openRecipeByData(${realIdx})"
      style="background:rgba(255,255,255,0.65);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border:0.5px solid rgba(59,130,246,0.18);border-radius:16px;padding:14px 16px;cursor:pointer;box-shadow:0 2px 8px rgba(30,58,95,0.06);-webkit-tap-highlight-color:transparent;display:flex;align-items:center;gap:12px;">
      <div style="width:4px;border-radius:2px;align-self:stretch;min-height:40px;flex-shrink:0;background:#3b82f6;"></div>
      <div style="flex:1;min-width:0;">
        <div style="font-size:16px;font-weight:600;color:#1e3a5f;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${r.title}</div>
        <div style="font-size:13px;color:#60a5fa;margin-top:3px;">${dispCat} · ${yieldTxt}${timeTxt}</div>
        ${editBtns}
      </div>
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(30,58,95,0.25)" stroke-width="2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg>
    </div>`;
  }).join('');
}
