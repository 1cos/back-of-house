const CACHE_NAME = 'boh-v650';
self.addEventListener('install', e => { self.skipWaiting(); });
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== 'boh-v650').map(k => caches.delete(k)))).then(() => self.clients.claim())); });
