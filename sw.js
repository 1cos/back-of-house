const CACHE_NAME = 'boh-v750';
self.addEventListener('install', e => { self.skipWaiting(); });
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== 'boh-v750').map(k => caches.delete(k)))).then(() => self.clients.claim())); });
