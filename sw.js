const CACHE_NAME = 'boh-v768';
self.addEventListener('install', e => { self.skipWaiting(); });
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== 'boh-v768').map(k => caches.delete(k)))).then(() => self.clients.claim())); });
