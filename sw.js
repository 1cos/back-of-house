const CACHE_NAME = 'boh-v735';
self.addEventListener('install', e => { self.skipWaiting(); });
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== 'boh-v735').map(k => caches.delete(k)))).then(() => self.clients.claim())); });
