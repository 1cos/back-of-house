const CACHE_NAME = 'boh-v714';
self.addEventListener('install', e => { self.skipWaiting(); });
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== 'boh-v714').map(k => caches.delete(k)))).then(() => self.clients.claim())); });
