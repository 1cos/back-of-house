const CACHE_NAME = 'boh-v722';
self.addEventListener('install', e => { self.skipWaiting(); });
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== 'boh-v722').map(k => caches.delete(k)))).then(() => self.clients.claim())); });
